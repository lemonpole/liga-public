...
export class API {
  ...
  public static async releases( repo: string ) {
    const url = [ API.apiBaseUrl, 'repos', repo, 'releases' ].join( '/' );
  ...
  public static getDownloadLink( assets: IAsset[] ) {
    ...
    const download_info = assets.find( item => item.browser_download_url.indexOf( download_ext || default_download_ext ) >= 0 );

    // if nothing was found, default to windows
    if( !download_info ) {
      const default_download_info = assets.find( item => item.browser_download_url.indexOf( default_download_ext ) >= 0 );
